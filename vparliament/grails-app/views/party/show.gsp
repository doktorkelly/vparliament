	...
	<body>
		...
		<div id="show-party" class="content scaffold-show" role="main">
			...
			<ol class="property-list party">
				...
				<li class="fieldcontain">
					<span id="issues-label" class="property-label"><g:message code="area.name.label" default="Issues for" /></span>
					<g:each in="${Area.findAll()}" var="area">
						<span class="property-value" aria-labelledby="issues-label">
							<g:link controller="issue" action="list"  params="[partyId: partyInstance.id, areaId: area.id]">
							${area?.encodeAsHTML()}</g:link>
						</span>
					</g:each>					
				</li>
			
				<%--
				<g:if test="${partyInstance?.issues}">
				<li class="fieldcontain">
					<span id="issues-label" class="property-label"><g:message code="party.issues.label" default="Issues" /></span>
						<g:each in="${partyInstance.issues}" var="i">
